...
def fetch_monthly_prices(ticker: str) -> pd.Series:
    """Fetch monthly closing prices safely."""
    ...
    # Pick price column
    for candidate in ["Adj Close", "Adj Close "+ticker, "Close", "Close "+ticker]:
...
# Compute YoY % and drop NaN
asx_yoy = (asx.pct_change(12) * 100).dropna()
spx_yoy = (spx.pct_change(12) * 100).dropna()
...
ax1.bar(range(len(asx_yoy)), asx_yoy.values, color="steelblue", alpha=0.8)
...
ax2.bar(range(len(spx_yoy)), spx_yoy.values, color="orange", alpha=0.8)
...
try:
    ...
    # Parse "Mar-05" as March 2005
    df_aupop["Date"] = pd.to_datetime(df_aupop["Period"], format="%b-%y", errors="coerce")
    df_aupop["Population"] = pd.to_numeric(df_aupop["Population"], errors="coerce")
    df_aupop = df_aupop.dropna(subset=["Date", "Population"]).sort_values("Date")

    # --- Plot ---
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(df_aupop["Date"], df_aupop["Population"], color="tab:blue", linewidth=2)
    ax.set_title("Australian Population Growth (2005–2013)")
    ax.set_xlabel("Year")
    ...
    aus_pop_stats.append(
        f"Population growth rose from {start_val:.2f}% in {df_aupop['Date'].dt.strftime('%b %Y').iloc[0]} "
        f"to {end_val:.2f}% in {df_aupop['Date'].dt.strftime('%b %Y').iloc[-1]} ({change:+.2f} ppts change)."
    )
...
# =========================================================
st.header("🧳 Net Migration by State")
